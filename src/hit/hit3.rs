...
use super::shader::{HIT_MISS_SOURCE, QUAD_FROM_DUAL_SOURCE, HONE_QUAD_SOURCE, HONE_TRACE_SOURCE, DUAL_FROM_UVS_SOURCE, TRACE_SOURCE};
...
pub struct HitBasis3 {
    ...
    dual_from_uvs:    WebGlProgram,   
    quad_from_dual:   WebGlProgram,
...
impl HitBasis3 { 
    pub fn new(facet_groups: Vec<Vec<FacetShape>>) -> Self {
        ...
        HitBasis3 {
            ...
            dual_from_uvs:       gpu.get_quad_program_from_source(DUAL_FROM_UVS_SOURCE).unwrap(),
            quad_from_dual:      gpu.get_quad_program_from_source(QUAD_FROM_DUAL_SOURCE).unwrap(),
    ...
    fn hone_to_hit_or_miss(&self) {
        ...
        self.draw_dual_from_uvs();
        self.draw_quad_from_dual();
        for _ in 0..3 {
    ...
    fn draw_dual_from_uvs(&self){
        ...
        self.gpu.set_uniform_1i(&self.dual_from_uvs, "uv_tex", 2);
        self.gpu.draw(&self.hone_buffer.as_ref().unwrap().dual);
    }

    fn draw_quad_from_dual(&self){
        self.gpu.gl.use_program(Some(&self.quad_from_dual));
        self.set_facet_uniforms(&self.quad_from_dual);
        self.set_ray_uniforms(&self.quad_from_dual, 3);
